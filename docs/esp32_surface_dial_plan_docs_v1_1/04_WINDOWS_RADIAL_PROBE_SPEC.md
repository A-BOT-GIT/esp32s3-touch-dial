...
Surface Dial / Radial Controller 的成功标准不是系统音量变化。

Consumer Control 音量键验证的是普通媒体键。Surface Dial 验证的是 Windows RadialController 输入系统。
...
- Windows 是否把 ESP32 当作 radial controller；
- 是否能收到 ButtonClicked；
- 是否能收到 RotationChanged；
- 菜单是否能出现；
- 当前 app 是否能获得 radial input。
...
- RadialController API 是 Windows 官方 wheel 交互入口；
- 比直接读 HID 更接近真实应用行为；
- 能验证菜单和 rotation 事件。
...
1. Windows 设备管理器是否出现 HID-compliant device；
2. 是否仍是旧设备名；
3. 是否连接稳定；
4. 是否默认禁用了 `setEncryptionLevel`；
5. 串口是否 `hid=sent`；
6. Report Reference 是否 `01 01`；
7. BLE notify value 是否 2 字节，不带 Report ID；
8. Report Map 是否使用 `System Multi-Axis Controller 0x0E`；
9. Dial 是否是 Relative；
10. Button 是否按下后释放；
11. Probe 是否前台获得焦点。
...
如果 RadialController API 不收事件，但设备被 HID 枚举，可以后续做 Raw HID probe：

- 枚举 HID devices；
- 读取 HID report descriptor；
- 打印 input report；
- 确认 Windows HID 层是否收到 `02 00` / `FE FF`。

Raw HID probe 是二级工具，不是第一优先级。
